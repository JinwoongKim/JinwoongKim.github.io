...
### Step 7: 빈 줄 두 개 → `<br>` 변환
**Obsidian에서 빈 줄 두 개로 띄워도 Jekyll에서는 하나로 합쳐지므로, 자동 변환한다.**
...
- 연속된 빈 줄 2개 이상 → 빈 줄 + `<br>` + 빈 줄로 변환
- front matter(---로 감싸진 부분)는 변환하지 않음
- 코드 블록(``` 또는 ~~~) 내부는 변환하지 않음

### Step 8: 맞춤법/띄어쓰기 검사
**게시 전 글 내용의 맞춤법과 띄어쓰기를 검사하고, 문제가 있으면 사용자에게 알려준다.**

규칙:
- 직접 수정하지 않음
- 문제 있는 부분을 목록으로 정리해서 사용자에게 보여줌
- 사용자가 확인 후 게시 진행 여부 결정

### Step 9: 글 내용 수정 및 이동
1. 태그 정리 (Step 6 규칙 적용)
2. 빈 줄 변환 (Step 7 규칙 적용)
3. `Filesystem:write_file` - 수정된 내용으로 `_posts/`에 저장 (Step 5의 파일명 사용)
4. 원본 `_posts_wip/` 파일은 `_trash/`로 이동
...
| 빈 줄 2개 이상 | `<br>` 태그로 자동 변환 |
| 맞춤법/띄어쓰기 | 검사 후 수정하지 않고 사용자에게 알려주기 |
