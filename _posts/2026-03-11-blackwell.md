...
일반적인 서빙 워크로드에서는 CUDA 추상화가 cross-die 통신을 완전히 처리해준다. 10 TB/s NV-HBI 대역폭 덕분에 대부분의 경우 성능 영향은 무시할 수 있는 수준이다. 다만 **커스텀 커널을 작성하거나 Nsight Compute로 프로파일링할 때**는 두 다이 간 데이터 이동 패턴을 알고 있으면 결과 해석에 도움이 된다.
...
| | B300 | B200 |
|---|---|---|
| Dense FP4 | ~7 PFLOPS | ~4.5 PFLOPS |
| Sparse FP4 | ~14 PFLOPS | ~9 PFLOPS |

NVIDIA 공식 블로그에 따르면 Blackwell Ultra는 "최대 15 PFLOPS의 고밀도 NVFP4 처리량, 동일 GPU에서 FP8 대비 3배"를 제공한다.
...
NVFP4의 정확도는 이미 검증됐다. NVIDIA는 MLPerf 훈련/추론 평가에서 **모든 LLM 테스트를 NVFP4로 제출**했고, 엄격한 벤치마크 정확도 요구사항을 충족했다. 512개 Blackwell Ultra GPU로 Llama 3.1 405B 사전 훈련을 64.6분에 완료(이전 라운드 대비 1.9배)한 결과도 NVFP4 기반이다.
...
**WHAT**: SM당 256KB TMEM (whitepaper 재확인 필요) — warp-synchronous 중간 결과 저장용. 레지스터와 비슷한 속도, 명시적 할당 필요.
...
1,200W짜리 칩 8개가 한 서버에 들어가면, 공기만으로는 열 방출이 물리적으로 불가능하다.
...
LLM 추론은 대부분 **memory-bandwidth bound**다. FLOPS보다 메모리 대역폭이 실질적인 throughput 상한을 결정한다.
...
B200과 B300의 메모리 대역폭은 동일하다. B300의 차별점은 **용량(288GB)**과 **NVFP4 연산 성능**이다. 대역폭이 같으므로 FP8 추론에서는 B200과 B300의 토큰 생성 속도(decode) 차이가 크지 않다. NVFP4를 사용해야 weight 크기가 절반으로 줄어 같은 대역폭으로 2배의 weight를 읽을 수 있고, 이것이 B300의 실질적 성능 이점이 된다.
...
MoE 모델(DeepSeek-R1 등)에서는 전통적인 Tensor Parallelism(TP)만으로는 최적이 아닐 수 있다. **Expert Parallelism(EP)**은 각 expert를 별도 GPU에 배치하여, 활성화된 expert만 해당 GPU에서 처리하는 방식이다.
...
B300의 288GB + NVLink 5.0(1.8 TB/s) 환경에서 EP가 특히 유리한 이유:
...
NVIDIA의 MoE 추론 최적화 블로그에 따르면, Blackwell에서 다음과 같은 SW 최적화가 EP 성능을 크게 끌어올린다:
- **Programmatic Dependent Launch**: 커널 실행 간 지연 시간 감소 — expert dispatch/combine 사이의 오버헤드를 줄임
- **All-to-All 통신 개선**: 중간 수신 버퍼를 제거하여 expert 간 데이터 교환 효율화
- 이 최적화들만으로 동일 플랫폼에서 3개월간 **GPU당 처리량 2.8배 증가**를 달성
