...
- 이러한 `kernel fusion` 도 단점이 있는데, 한 번 합쳤던 코드를 다시 나누기 어려워 상황에 맞게 유동적으로 변경하긴 힘들다는 것이다. 또한 하드웨어 및 워크로드 종속적인 업무들이 있어, 주어진 업무에 맞게 `kernel fusion` 을 다 다르게 해야 하는 경우가 많다.
- 따라서 다시 나누지 않을 것 같은, 폭넓게 적용되는 
