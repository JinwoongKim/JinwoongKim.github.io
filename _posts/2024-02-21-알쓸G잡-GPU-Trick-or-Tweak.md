...
- 일반적으로 우리는 좌측과 같이 동시에 처리 될 것이라고 생각하지만, 앞서 설명했듯이 하나의 warp는 하나의 명령어만 수행하기 때문에 짝수번 ID를 가지는 쓰레드가D 를 가지는 녀석들을 분기문을 태운다면, 아래 처럼 일부 쓰레드가 코드를 수행할때 나머지 쓰레드가 다른 구문을 수행하는게 아니라, 기다리는 상황이 발생한다.
