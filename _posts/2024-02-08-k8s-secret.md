...
위의 `pod.yaml`에 두 가지를 추가해주면 된다. 먼저, `spec.volumes` 에 시크릿을 위한 볼륨을 생성해주고, 그 다음에 `spec.containers.volumeMounts` 를 이용해 해당 볼륨을  `"/etc/secret"` 밑에 마운트 해주면 된다. 이러면 팟 안에서 `/etc/secret/mypassword` 에 접근하여 값을 가져 올 수 있다.
