...
자, 이제 시크릿을 만들었으니, 팟에 마운트해서 사용해보자. 팟이라고 했지만 아래 예시는 `deployment` 인데, 시크릿을 만든다는 건, 팟이 지속적으로 배포되는 경우에 유연하게 대처하기 위함이기 때문..d 내가 갖고 있는 코드가  `deployment` 이기 때문이다. 팟으로 띄우고 싶으신 분들은 첫번째 spec 을 날리고 kind 를 바꾸면 된다.
...
`spec.spec.containers.volumeMounts` 를 이용해 해당 볼륨을  `"/etc/secret"` 밑에 마운트 해준다.

이렇게 팟 안에서 `/etc/secret/mypassword` 에 접근하여 값을 가져 올 수도 있다.

만약 환경변수로 만들고 싶다면, `spec.spec.containers.env` 에 마운트된 시크릿의 키를 환경 변수로 셋팅해주면 된다.
