...
```
podman exec --detach-keys="ctrl-X" -it 663c9eedea04 bash
```


매번 이렇게 치기 귀찮아서 아예 podmand의 detach key map을 바꾸는 설정을 찾았다!
...
detach_keys에 empty string을 넣어줬더니 다시 원복이 된다. 빈 문자열보단 안 쓸 것 같은 키를 할당하자.