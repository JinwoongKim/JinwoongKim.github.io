...
첫째, 동작하지 않는 코드를 너무 많이 뱉어낸다. 처음에는 모델의 성숙도가 떨어져서라고 생각했는데, 그보다는 '내가 프롬프트를 어설프게 작성해서'가 주요 원인 같았다. 주요 기능등은 곧잘 구현하고 잘 동작하지만, 언어의 숙련도가 낮기 때문에 나조차도 어떻게 해야할지 몰라 코드가 운영 할 수 있는 정도의 수준으로 코드를 작성하기는 무리가 있었다.
