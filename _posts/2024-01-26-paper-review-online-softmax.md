...
기존(<코드2>)에서는 max 값을 구하려고 모든 xj 에 접근하면서 메모리 접근 한 번씩, 그리고 dj 를 구하느라 다시 한 번 xj에 접근하는데, 위의 코드(<코드3>)에서는 xj 에 한 번 엑세스하고 dj 를 구해 버리므로 메모리 접근 횟수가 한 번 줄어든다.

이렇게 함으로써 yi 당 메모리 접근 횟수가 4에서 3이 된다.
