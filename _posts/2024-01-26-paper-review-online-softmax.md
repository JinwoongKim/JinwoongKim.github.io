...
원래는 max 값을 구하고, 그 값을 이용해서 dj 를 구했다면, 여기서는 실시간으로 현재 최댓값을 이용해서 dj  를 구하고 나중에 그 값을 정상화(?) 해준다.

기존(<코드2>)에서는 max 값을 구하려고 모든 xj 에 접근하면서 메모리 접근 한 번씩, 그리고 dj 를 구하느라 다시 한 번 xj에 접근하는데, 위의 코드(<코드3>)에서는 xj 에 한 번 엑세스하고 dj 를 구해 ㅂ
이렇게 함으로써 메모리 접근 횟수가 4에서 3이 된다.

처음에는 이 부분이 정말 이해가 안 갔다. 아래 예제를 보면 바로 이해가 갈 것이다.
