...
- 이를 위해서는 알고리즘 적으로도 조치가 필요한데, 가장 흔한 것이 타일링 이다. 타일링이란 shared-nothing 구조의 단순히 2차원 문제를 divide-and-conquor 하는 방식이다. 이때, 타일의 사이즈를 SBM 사이즈로 맞추는 식으로. → FlashAttention
