...
- H100 SXM5(NVSwitch) 는 132 개의 SM 을 갖고 있으며, PCIe 버전은 114개의 SM 을 갖고 있다.
	...
	- 예전에 Nvidia 관계자에게 왜 on-chip 메모리 사이즈 늘리지 않느냐고 물어본 적이 있었는데, 메모리 늘릴 공간 있으면 코어 하나라도 더 설치한다고 했었다.
...
- H100의 경우 하나의 SM이 4개의 SP로 구성되어 있다.
	...
	- 하지만, 다른 SM에 있는 코어들과 데이터를 교환하고 싶을때는 글로벌 메모리를 사용해야 해서 성능 저하가 있었는데, 이번 Hopper 아키텍쳐에선 
