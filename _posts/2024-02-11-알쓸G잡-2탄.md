...
초기 GPU는 CPU와 부동 소수점 연산 명령어도 달랐고, 오차가 꽤 컸다. 위에서 언급했듯이 초당 최소 30번 값을 뿌려 주었기 때문에, 값이 조금 틀려서 색이 다르게 표현되도 인간의 눈이 알아차리긴 어려웠기 때문이다. 하지만 과학 연산 용으로 사용되는 요즘 GPU는 연산 정확도 및 다른 CPU/GPU 왕
{: .notice--info}
