...
- 이를 위해서는 알고리즘 적으로도 조치가 필요한데, 가장 흔한 것이 타일링 이다.
	- 타일링이란 shared-nothing 구조의 문제를 단순히 분할 정복 하는 방식이다. (딥러닝의 data parallel 같은 것) 이때, 타일의 사이즈를 on-chip 사이즈로 맞추는 식으로 하면 된다.
	- 예를 들어 우리가 1부터 100까지의 숫자에 각각 1을 더한다고 가정해보자. 우리의 GPU는 SM이 10개 있고, 각각의 SM의 메모리에는 숫자가 최대 4개씩 들어간다고 치면, 총 25개의 타일을 생성해서 SM 들에게 동적으로 배분하면 된다.
