...
<img width="900" alt="image" src="https://github.com/JinwoongKim/JinwoongKim.github.io/assets/12505517/99479a23-de1f-4aa4-ada7-d540489dd443">

<img width="900" alt="image" src="https://github.com/JinwoongKim/JinwoongKim.github.io/assets/12505517/fe28790d-f431-41dd-a428-c0d3390f2fb3">
...
<img width="578" align="center" alt="Pasted image 20240213214647" src="https://github.com/JinwoongKim/JinwoongKim.github.io/assets/12505517/30a28b75-a900-4ecd-9a2c-4e4fdc157d73">
...
- 본 글에서는 Tesla, IDC 용 GPU 를 타겟으로 진행 하겠다.

# 2.  GPU Architecture
## CPU vs. GPU 구조 


<img width="618" alt="Pasted image 20240206111150" src="https://github.com/JinwoongKim/JinwoongKim.github.io/assets/12505517/18535d38-1afd-4b01-a6b4-4c6642475c2d">

<img width="1802" alt="Pasted image 20240206110349" src="https://github.com/JinwoongKim/JinwoongKim.github.io/assets/12505517/0f49f423-d48d-4869-810a-6520bdc179f0">

<img width="351" alt="Pasted image 20240206121605" src="https://github.com/JinwoongKim/JinwoongKim.github.io/assets/12505517/b99bf320-6c54-4920-a730-338bc9a3d509">

![[blog/images/Pasted image 20240206111150.png]]

출처 : https://docs.nvidia.com/cuda/cuda-c-programming-guide/index.html

- CPU와 GPU의 구조를 비교한 그림이다. 개론적인 그림이지만 딱 봐도 CPU 대비 GPU 코어의 갯수가 압도적으로 많다.
...
- 하지만 이러한 구조 덕분에 CPU 보다 더 병렬처리에 적합하다. 
	- 대신 높은 코어를 성능을 요구하는 명령어보단 단순한 사칙연산 등에 더 적합하다.
...
- H100 SXM5(NVLink) 는 132 개의 SM 을 갖고 있으며, PCIe 버전은 114개의 SM 을 갖고 있다.
	- 참고로 A100의 SM의 갯수는 108개 이다.

![[blog/images/Pasted image 20240206110349.png]]
출처 : nvidia-h100-tensor-core-hopper-whitepaper.pdf
## SM 구조

![[blog/images/Pasted image 20240206110314.png]]
출처 : nvidia-h100-tensor-core-hopper-whitepaper.pdf

- H100은 132개의 SM으로 구성되어 있고, SM은 총 4개의 SP로 구성되어 있다. SM내 SP들은 L1 캐쉬와 Shared Memory를 공유한다.
	- Shared Memory는 SM내 SP들이 데이터를 공유하기 위해 사용한다.
		- H100에선 SM끼리 통신도 함; ㅎㄷ

- FlashAttention 의 메인 아이디어? 중 하나가 HBM 접근 최소화ㅏ는 것. SBM 접근
	- 심지어 HBM 가느니 재계산함. 그게 싸니까 ㅇㅇ
- GPU 프로파일링?
- 예전내용을 그림으로 요약?

- 일반적으로 On-chip 메모리라고 하면 SM 안에 있는 Shared Memory를 뜻하고 Off-ship 메모리라고 하면 밖에 있는 (Global) Memory를 뜻한다.
	- GPU DRAM은 Global Memory, Device Memory 등으로 불리운다.
- On-chip과 off-chip 메모리의 접근속도는 약 7~8배 정도가 차이 난다. 물론 대역폭은 off-chip이 더 높다.
- 각 SP 안에는 여러 종류의 GPU 코어와 레지스트리 등으로 구성되어 있다.
	- 텐서코어는 행렬곱을 가속화한 장치
	- 일반적으로 GPU 코어는 1 clock 동안 하나의 부동소수점 연산을 하는데 반해, 텐서코어는 4x4 행렬곱을 수행함.
		- 이때 입력은 FP16, 출력은 FP32 이라 mixed precision 이라고 부르기도 함

occupancy table? excel?

![[blog/images/Pasted image 20240214124645.png]]


# 3. GPU Thread Hierarchy

- 앞서 GPU의 물리적 구조에 대해 알아보았다. 이제 논리적 구조에 대해 알아보자.
- CPU는 쓰레드가 1차원인데 반해, GPU는 3차원 x 3차원 임
- Grid of Thread Block, 각 Thread BlocK 안
- 근데 Hopper 부터 하나의 개념이 더 생김. Thread block cluster
![[blog/images/Pasted image 20240206121933.png]]


- CPU는 쓰레드가 1차원인데 반해, GPU는 렌더링 때문에 3차원까지 지원 함
- 게다가 구조도 Grid -> Block -> Thread Block Cluster -> Thread Block -> Warp -> Thread
	- 3D 랜더딩 때문 같음. 우리도 매트릭스 연산떄문에 2차원까지는 많이 씀
- 당장은 블럭, 쓰레드 이 두 가지만 기억하자.
	- 예를 들어 블럭2깨에 쓰레드 3개 이러면, 블럭2개가 만들어지고 각각 블럭 안에 쓰레드가 3개씩 생성됨
- 각각의 블럭은 동시에 하나의 SM에서만 수행 됨
- 즉, 아래 그림처럼 블럭을 총 8개를 생성하면, SM 이 두 개인 GPU 는 왼쪽과 같이, 4개인 GPU는 오른쪽 같이 실행 됨
	- 물론 SM의 상태에 따라 블럭이 다이나믹하게 할당 됨.
		- H100의 경우 하나의 SM이 32개의 블럭을 홀딩?할수있음 A40은 16개임.A100은 똑같이 32

![[blog/images/Pasted image 20240206133458.png]]
