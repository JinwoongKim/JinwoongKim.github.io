...
#### 용어 설명
- Compute capability (이하 CC): GPU는 OS가 없지만, OS 버전 같은 거라 생각하면 된다. 물리적인 것 보단 논리적인 능력치라고 보면 편하다. 예를 들어 CC에 따라 GPU 코어가 동시에 몇개의 쓰레드를 수행 할 수 있는지 등이 다르다.

adsf
![](../images/Pasted%20image%2020240213220000.png)


![[/images/Pasted image 20240206115740.png]]
asdf

![[/blog/images/Pasted image 20240206115740.png]]
출처 : https://en.wikipedia.org/wiki/CUDA
- Micro-architecture : GPU의 근간이 되는 아키텍쳐 이름이다. 같은 아키텍쳐에서 여러 CC 가 있을 수 있다. 예를 들어 Ampere 아키텍쳐에서 A100은 CC가 8.0이고 A40은 8.6이다. 그렇다고 아키텍쳐가 변한다고 CC가 1씩 올라가는 것도 아니다. Volta는 7.0~7.2였고 Turing은 7.5였다. 그리고 CC가 더 높다고 무조건 좋은 건 아니다. 하지만 일반적으로 CC가 높으면 지원하는 기능들이 더 많다.
![[../images/Pasted image 20240213214647.png]]
- GPUs : 코드 네임. 뒤에 나오는 GeForce, Quadro, Tesla 등의 기반이 되는 GPU 구조.
	- 예를 들어 똑같은 에스프레소샷에 따듯한 물타면 뜨아, 얼음타면 아아 되듯이, 같은 GPU 아키텍쳐를 이용해서 그래픽 관련 모듈 설치하고 RT 코어 달고 하면 게임 용 GeFore, 그래픽 포트 없애고 GPU 코어를 더 집적하고, DRAM 더 달아서 과학연산 용으로 만들면 Tesla, 이런 식으로 생각하면 된다.
	- Quadro는 GeForce와 Tesla를 섞은 느낌이다. 화면 출력도 되고 메모리 집적도도 어느 정도 높다. 보통 (24시간 실험하는) 대학원생이나 개인 장비를 소유하는 연구자들이 출력 및 관리가 용이해 많이 쓴다. Tesla 는 서버렉에 설치하는 형태가 일반적이고 Quadro는 데스크탑 형태이기 때문이다. 하지만 LLM의 경우 여러개의 GPU 혹은 서버를 엮어야 해서 Quadro 를 많이 쓰지는 않는다.
	- Jetson 라인은 자율주행 자동차에 들어가는 라인
- 본 글에서는 Tesla, IDC 용 GPU 를 타겟으로 진행 예정
...
- CPU와 GPU의 구조를 비교한 그림이다. CPU 대비 GPU 코어의 갯수가 압도적으로 많다.
...
- 그래서 CPU 보다 더 병렬처리에 적합한데, 연산이 어려운거보단 단순한것, float operation. 을 잘 한다.
- GPU DRAM은 그래서 대역폭이 매우 크다.
...
- H100 SXM5 는 132 SM 을 갖고 있으며, PCIe 버전은 114개의 SM 을 갖고 있다.
	- 참고로 A100의 SM의 갯수는 114개 이다.
...
- H100의 SM은 총 4개의 SP로 구성되어 있으며 각 SP는 L1 캐쉬와 Shared Memory를 공유한다.
- 메모리
	- L1 캐시
	- 
- 일반적으로 On-chip 메모리라고 하면 SM 안에 있는 메모리들을 뜻한다.
- On-chip과 off-chip 메모리는 7~8배 정도의 latency 차이가 난다. 물론 대역폭은 off-chip이 더 높다.
- 하지만 shared memory 보다 register file이 더 빠르다.
- RF 은 쓰레드가 접근. SM은 서로 교환하고 막 그럼.
- SM/L1 공유 구조 설명 : 
