...
- SP 내부에는 Register File 이 있는데, 이는 GPU 코어들을 수행하는 쓰레드들의 캐시라고 생각하면 된다.
	- 몇몇 특수한 경우에는 이러한 Register File 을 잘 활용하는 것이 주요 성능 포인트인데, 보통은 Shared Memory만 잘 활용해도 많은 성능 향상을 얻을 수 있다.
	- 서로 다른 SP 간의 데이터교환을 위해 shared memory를 사용하며
- 메모리
	- L1 캐시
	- 
