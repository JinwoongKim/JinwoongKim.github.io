...
#### 용어 설명
- Compute capability : GPU는 OS가 없지만, OS 버전 같은 거라 생각하면 된다. 물리적인 것 보단 논리적인 능력치라고 보면 편하다. 예를 들어 compute capability에 따라 GPU 코어가 동시에 몇개의 쓰레드를 수행 할 수 있는지 등
![[blog/images/Pasted image 20240206115740.png]]
출처 : https://en.wikipedia.org/wiki/CUDA
- Micro-architecture : GPU 모델의 아키텍쳐 이름이다. 같은 아키텍쳐에서 여러 CC 가 있을 수 있다. 예를 들어 Ampere 아키텍쳐에서 A100은 CC가 8.0이고 A40은 8.6이다. 그런데 CC가 더 높다고 무조건 좋은 건 아니다.
