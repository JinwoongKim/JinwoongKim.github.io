...
- Compute capability : GPU는 OS가 없지만, OS 버전 같은 거라 생각하면 된다. 물리적인 것 보단 논리적인 능력치라고 보면 편하다. 예) CC에 따라 GPU 코어나 칩셉이 동시에 몇개의 쓰레드를 수행 할 수 있는지 등이 결정됨
![[blog/images/Pasted image 20240206115740.png]]
출처 : https://en.wikipedia.org/wiki/CUDA

- Micro-architecture : 모두들 친숙할듯. 아키텍쳐 이름.
- GPUs : 코드 네임. 뒤에 나오는 GeForce, Quadro, Tesla 등의 기반이 되는 GPU 구조.
	- 예를 들어 똑같은 에스프레소샷에 따듯한 물타면 뜨아, 얼음타면 아아 되듯이, 같은 GPU 아키텍쳐를 이용해서 그래픽 관련 모듈 설치하고 RT 코어 달고 하면 게임 용 GeFore, 그래픽포트 없애고 GPU 코어를 더많이 집적하고, DRAM 더 달아서 과학연산 용으로 만들면 Tesla, 이런 식으로 생각하면 된다.
...
- 초기 GPU는 모니터 화면에 값을 연산해서 뿌려주는 용도 였기 때문에 이러한 ManyCore 구조를 가지게 되었다.
