...
크게 두 가지 특성이 이러한 것들을 야기하는데, 첫째 하나의 request를 위해 꼭 **연속**된 주소의 메모리를 할당 받는 연속성, 둘째, 할당받은 메모리를 하나의 request가 잠시라도 공유하지 않고 **독점** 하는 점유성을 꼽는다.
...
여기서 만약, parallel sampling을 하거나 beam search를 하게 된다면 더욱 메모리 사용량이 증가한다.

이러한 관리체계가 메모리 효율을 어떻게 떨어뜨릴까? 좀 더 자세히 알아보자.

## 야기되는 문제#1: 잦은 메모리 복사

Parallel sampling 및 beam search의 경우, 프롬프트 또는 기존 생성한 토큰들의 KV 값이 같은 경우가 종종 있다. 그럼에도 불구하고 기존 방식들은 새로운 토큰을 생성하기 위해, 기존 프롬프트나 토큰들의 KV 값이 **연속된 메모리에 있어야 하기 때문에**, 이미 메모리에 있는 값이라 하더라도 또다시 복사를 한다.
...
마지막으로 적절한 KV block 사이즈를 찾는 것은, 실험을 통해 값을 공유해주고 있다. 하지만, 사용자의 환경에 따라 달라 질 수 있으므로, 이는 사용하는 유저가 변경 가능하도록 파라미터화 하였다.
