...
Parallel sampling 및 beam search의 경우, 프롬프트 또는 기존 생성한 토큰들의 KV 값이 같은 경우가 종종 있다고 한다.

그럼에도 불구하고 기존 방식들은 새로운 토큰을 생성하기 위해, 기존 프롬프트나 토큰들의 KV 값이 **연속된 메모리에 있어야 하기 때문에**, 이미 메모리에 있는 값이라 하더라도 또 복사를 하여 사용 해야 한다.
