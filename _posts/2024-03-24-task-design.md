...
저기에는 지금 `ROOT CAUSE`가 빠져있다. `모델러들간 중복되는 데이터가 왜 많은지`, 이 모든 문제가 어디서 시작되었고, 왜 저것이 원인으로 지목되었는지를 파악해야 한다. 이 문제는 알고보면 모델러들끼리의 커뮤니케이션 부족으로 발생한 문제일 수도 있는 것이다. 모델러들끼리 의사소통을 해서 아주 쉽게 문제를 해결 할 수도 있는 것이다.
