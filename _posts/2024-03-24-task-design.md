...
사실 대부분의 신입사원들은 이렇게 업무 정의를 마칠 것이다. 하지만 조금만 더 나아간다면, `왜 NAS를 A 서버에 설치`하라고 했지?라고 의문을 품게 될 것이다. 여기서 나오게 되는 개념이 task design의 핵심 요소인 `WHY` 이다. 
...
`WHY(w/ ROOT CAUSE)` : `모델러들간 소통 부재로 중복 데이터가 많이 발생함`<br>
`WHAT` : `데이터 공유 시스템 등의 무언가`<br>
`HOW` : `A서버에 NAS 설치`<br>
